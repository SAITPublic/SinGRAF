...
from utils.planes import sample_from_planes
from utils.planes import generate_planes
...
class NerfFeatureGenerator(nn.Module):
    """
    modified version of NerfSimpleGenerator not to use coordinate (without skips)
    """
    def __init__(self, n_layers=8, channels=256, out_channel=3, z_dim=128, omega_dir=4):
        super().__init__()

        self.from_dirs = PositionalEncoding(in_dim=3, frequency_bands=omega_dir)

        self.layers = nn.ModuleList(
            [EqualLinear(in_channel=z_dim, out_channel=channels, activate=True)]
        )
    ...
    def forward(self, z, coords=None, viewdirs=None):  # NOT using coords
        """Forward pass.
...
class NerfFeatureNoDirGenerator(nn.Module):  # NOT using coords nor viewdirs
    """
    modified version of NerfSimpleGenerator not to use coordinate nor view direction (without skips)
    """
    def __init__(self, n_layers=8, channels=256, out_channel=3, z_dim=128):
        super().__init__()
    ...
    def forward(self, z, coords=None, viewdirs=None):  # ignoring coords and viewdirs
        """Forward pass.
...
class SceneGenerator(nn.Module):
    """NeRF scene generator. Implementation of original GSN
...
class TriplaneGenerator(SceneGenerator):
    """NeRF scene generator using Triplane representation. Basic functions are inherited from SceneGenerator
    ...
    def __init__(
        ...
        coordinate_scale=None,
        alpha_activation='softplus',
        local_coordinates=None,  # NOT used (IGNORED)
        hierarchical_sampling=False,
        density_bias=0,
        **kwargs
    ):
        super().__init__(nerf_mlp_config, img_res, feature_nerf, global_feat_res, coordinate_scale, alpha_activation, False, hierarchical_sampling, density_bias, **kwargs)
        
        self.n_planes = 3
        self.latent_code = nn.Parameter(torch.zeros(1, 512), requires_grad=False)
    ...
    def query_network(self, xyz, local_latents, viewdirs):
        ...
        # sample from Triplane representation
        local_latents = local_latents.reshape(local_latents.shape[0], self.n_planes, -1, local_latents.shape[-2], local_latents.shape[-1])
        sampled_local_latents, _ = sample_from_planes(self.plane_axes, local_latents, xyz.reshape(B, n_samples*samples_per_ray, -1))
        N, n_planes, M, C = sampled_local_latents.shape
        sampled_local_latents = sampled_local_latents.permute(0, 2, 1, 3).reshape(N, M, n_planes*C)

        xyz = xyz.reshape(-1 , 3)
        viewdirs = viewdirs.reshape(-1, 3) if viewdirs is not None else None

        sampled_local_latents = sampled_local_latents.reshape(-1, n_planes*C)
        rgb, alpha = self.local_generator(z=sampled_local_latents, coords=xyz, viewdirs=viewdirs)
    ...
    def get_ray_bundle_batch_perspective(self, height: int, width: int, fx, fy, tform_cam2world: torch.Tensor):
    ...
    def get_ray_bundle_batch_cylindric(self, height: int, width: int, fx, fy, tform_cam2world: torch.Tensor):
        B = tform_cam2world.shape[0]
        x = torch.arange(width, dtype=tform_cam2world.dtype, device=tform_cam2world.device).to(tform_cam2world)
        y = torch.arange(height, dtype=tform_cam2world.dtype, device=tform_cam2world.device)
        ii, jj = torch.meshgrid(x, y)
        ...
        directions = torch.stack([(ii - width * 0.5) / fx, (jj - height * 0.5) / fy, -torch.ones_like(ii)], dim=-1)
        directions[:, :, 0::2] = nn.functional.normalize(directions[:, :, 0::2], dim=-1)
        directions = directions.view(1, -1, 1, 3).repeat(B, 1, 1, 1).view(-1, 1, 3)
        tform_cam2world = tform_cam2world.unsqueeze(1).repeat(1, height * width, 1, 1).view(-1, 4, 4)
    ...
    def get_ray_bundle_batch_orthographic(self, height: int, width: int, fx, fy, tform_cam2world: torch.Tensor, far):
        B = tform_cam2world.shape[0]
        x = torch.arange(width, dtype=tform_cam2world.dtype, device=tform_cam2world.device).to(tform_cam2world)
        y = torch.arange(height, dtype=tform_cam2world.dtype, device=tform_cam2world.device)
        ii, jj = torch.meshgrid(x, y)
        ...
        origins = torch.stack([(ii - width * 0.5) / fx, (jj - height * 0.5) / fy, torch.zeros_like(ii)], dim=-1) * far
        origins = origins.view(1, -1, 1, 3).repeat(B, 1, 1, 1).view(-1, 1, 3)
        directions = torch.stack([torch.zeros_like(ii), torch.zeros_like(ii), -torch.ones_like(ii)], dim=-1)
        directions = directions.view(1, -1, 1, 3).repeat(B, 1, 1, 1).view(-1, 1, 3)
        tform_cam2world = tform_cam2world.unsqueeze(1).repeat(1, height * width, 1, 1).view(-1, 4, 4)

        ray_directions = torch.sum(directions * tform_cam2world[:, :3, :3], dim=-1).view(B, height, width, 3)
        ray_origins = torch.sum(origins * tform_cam2world[:, :3, :3], dim=-1).view(B, height, width, 3) + tform_cam2world[:, :3, -1].view(B, height, width, 3)
        return ray_origins, ray_directions

    def render(self, local_latents, render_params=None, xyz=None, mode='perspective'):  # for rendering and visualization with various camera settings
        return_alpha_only = True if xyz is not None else False
        ...
        if render_params is not None:
            ...
            if mode == 'orthographic':
                ray_origins, ray_directions = self.get_ray_bundle_batch_orthographic(H, W, fx, fy, tform_cam2world, far)
            elif mode == 'cylindric':
                ray_origins, ray_directions = self.get_ray_bundle_batch_cylindric(H, W, fx, fy, tform_cam2world)
            else:
                ray_origins, ray_directions = self.get_ray_bundle_batch_perspective(H, W, fx, fy, tform_cam2world)
            ...
            xyz = ro[..., None, :] + rd[..., None, :] * z_vals[..., :, None]
            viewdirs = rd
